...
window.getIndikatorById = function(id) {
  ...
  // Sadar-jenjang: utamakan dataset sesuai jenjang aktif (RA punya dataset sendiri).
  const jList = (window.getInstrumenByJenjang ? window.getInstrumenByJenjang(window.PKKM_JENJANG_AKTIF) : null) || window.PKKM_KOMPONEN || [];
  const k = jList.find(x => x.code === code) || (window.PKKM_KOMPONEN || []).find(x => x.code === code);
  if (!k) return null;
...
window.getIndikatorTampil = function (ind, indikator_id, jenjang) {
  ...
  if (indikator_id && j && window.InstrumenOverride) {
    ...
    if (ov) {
      ...
      if (ov.bukti != null) out.bukti = ov.bukti;
      if (ov.penggalian != null) out.penggalian = ov.penggalian;
      if (ov.rubrik != null) out.rubrik = ov.rubrik;
    }
  }
  return out;
};
